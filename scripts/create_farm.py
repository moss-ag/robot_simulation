...
block_offset = 0 # Distance in meters (ideal = 3) to add block offset, by default odd no. of blocks are staggered 
...
def compute_grid_two(x_max,blocks):
    ...
    while count<blocks:
        ...
        while(x <= x_max and count<blocks):
            cord_m.append(1)
            row_l_i.append(((args.num_trees_row[count])-1)*row_width_spacing) #length of row
            count = count+1
            x=x+1
        x=1
...
for r in range(1,len(rl)): # Row lengths
    rl[r]=[sum(n) for n in zip_longest(rl[r-1], rl[r], fillvalue=0)] # Cummulative row length
...
def make_plane(rid, cid, xo,yo,xt,yt): 
    ...
    plane_str = get_plane_model(model_name,link_name,visual_name,(xo+xt)/2,(yo+yt)/2,abs(xo-xt)+exmud/2,abs(yo-yt)+exmud)
...
for rows in range(len(y_a)):
    ...
    while(ind<len(b_cords[rows])):
        ...
        for tr in range(0,args.num_trees_row[rows*len(b_cords[rows])+ind]):
            ...
            y_pos_m = [(tr*slope)+(rows*block_offset+element) for element in y_pos]
            
            x_pos = [(tr*row_width_spacing) + abs(random.gauss(mean,sigma)) for y_pos in y_pos_m]

            # Centering around origin
            if rows!=0:
                x_pos_m = [-shift_x+element + (rows*block_to_block_spacing)+rl[rows-1][ind] for element in x_pos]
            else:
            ...
            for i in range(0,len(x_pos_m)):
                make_row(rid = rows,cid=ind,tree_r = tr,idx=i, x_pos = x_pos_m[i],y_pos=y_pos_m[i])
