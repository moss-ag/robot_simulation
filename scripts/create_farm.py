...
parser.add_argument(
    '--num_blocks', '-nb', type=int, default=1,)
parser.add_argument(
    '--num_rows', '-nr', nargs='+', default=3,
    help='list of number of rows to use, 1 val per block. Default: 3')
parser.add_argument(
    '--num_trees_row', '-ntr', nargs='+', default=5,
    help='list of number of trees in each row to use, 1 val per block. Default: 5')
parser.add_argument(
    '--filename', '-f', type=str, default='worlds/spraybot.world',
...
if args.num_rows is not None:
    ...
    if len(args.num_rows) != 1 and len(args.num_rows) != args.num_blocks:
       raise ValueError(
            'Length of number of rows should be 1 (fixed val) or same as number of blocks')
...
if args.num_trees_row is not None:
    ...
    if len(args.num_trees_row) != 1 and len(args.num_trees_row) != args.num_blocks:
       raise ValueError(
            'Length of number of trees in a row should be 1 (fixed val) or same as number of blocks')
...
def make_row(y_ind,y_pos, num_trees_row):
    ...
    Parameters:
        ...
        num_trees_row (int): the number of trees in the row
    '''

    row_length = (num_trees_row - 1) * row_length_spacing
    for index, x_pos in enumerate(np.linspace(-row_length / 2, row_length / 2, num_trees_row)):
        name = 'tree_{}_{}'.format(y_ind,index)
        tree_str = get_tree_model(
            name=name, x=x_pos,
            y=y_pos - gazebo_left_offset, z=0)
